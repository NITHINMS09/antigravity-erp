...
router.get('/', auth_1.authenticate, async (_req, res) => {
    try {
        ...
        // Daily sales
        const dailySales = await prisma_1.default.invoice.aggregate({
            where: { invoiceDate: { gte: today } },
            _sum: { grandTotal: true },
            _count: true,
        });
        // Weekly sales
        const weeklySales = await prisma_1.default.invoice.aggregate({
            where: { invoiceDate: { gte: weekAgo } },
            _sum: { grandTotal: true },
            _count: true,
        });
        // Monthly sales
        const monthlySales = await prisma_1.default.invoice.aggregate({
            where: { invoiceDate: { gte: monthAgo } },
            _sum: { grandTotal: true },
            _count: true,
        });
        // Pending payments
        const pendingPayments = await prisma_1.default.invoice.aggregate({
            where: { paymentStatus: { not: 'paid' } },
            _sum: { dueAmount: true },
            _count: true,
        });
        // Low stock alerts
        const lowStockItems = await prisma_1.default.stock.findMany({
            where: {
                quantity: { lte: 0 }
            },
        ...
        const allStock = await prisma_1.default.stock.findMany({
            include: { material: true },
        });
        const lowStockAlerts = allStock.filter(s => s.quantity <= s.minLevel);
        // Recent expenses
        const monthlyExpenses = await prisma_1.default.expense.aggregate({
            where: { createdAt: { gte: monthAgo } },
            _sum: { amount: true },
        });
        // Worker expenses this month
        const workerPayments = await prisma_1.default.workerPayment.aggregate({
            where: { createdAt: { gte: monthAgo } },
            _sum: { amount: true },
        });
        // Transport expenses this month
        const transportExpenses = await prisma_1.default.transport.aggregate({
            where: { createdAt: { gte: monthAgo } },
            _sum: { totalCost: true },
        });
        // Purchase costs this month
        const purchaseCosts = await prisma_1.default.purchase.aggregate({
            where: { createdAt: { gte: monthAgo } },
            _sum: { grandTotal: true },
        });
        // Recent invoices
        ...
        // Bakery daily sales
        const bakeryDailySales = await prisma_1.default.bakerySale.aggregate({
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const daySales = await prisma_1.default.invoice.aggregate({
