...
export default function WorkersPage() {
  ...
  const fetchWorkers = async () => {
    try { const d = await api.get('/workers'); setWorkers(d.workers); }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 max-w-[1400px] mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h1 className="text-2xl font-bold text-zinc-100">Workers</h1><p className="text-sm text-zinc-500">Track daily punches, attendance, salary & payments</p></div>
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowAddWorker(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 text-white text-sm font-medium shadow-lg shadow-orange-500/25"><Plus className="w-4 h-4" /> Add Worker</button>
        </div>
      </div>
      ...
      {showAddWorker && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowAddWorker(false)}>
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0f0f1a] p-6" onClick={e => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-zinc-100 mb-4">Add Worker</h2>
            <form onSubmit={handleAddWorker} className="space-y-3">
              ...
              <div><label className="block text-xs text-zinc-500 mb-1">Rate per Punch (₹)</label><input type="number" step="0.01" value={addForm.ratePerPunch || ''} onChange={e => setAddForm({ ...addForm, ratePerPunch: +e.target.value })} className="w-full px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-zinc-200 text-sm outline-none" /></div>
      ...
      {showTeamWork && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowTeamWork(false)}>
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0f0f1a] p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-zinc-500 mb-4">Record punches for multiple workers at once</p>
            <form onSubmit={handleTeamWork} className="space-y-4">
                ...
                <div className="space-y-1 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                  {workers.map(w => (
                    <label key={w.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 cursor-pointer">
                      ...
                      />
                      <span className="text-sm text-zinc-200">{w.name} <span className="text-zinc-500 text-xs">(₹{w.ratePerPunch}/punch)</span></span>
                    </label>
                  ))}
                </div>
              </div>

              <div><label className="block text-xs text-zinc-500 mb-1">Total Punches (Will be applied to EACH selected worker)</label>
                <input type="number" value={teamForm.punches || ''} onChange={e => setTeamForm({ ...teamForm, punches: +e.target.value })}
                  className="w-full px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-zinc-200 text-sm outline-none" placeholder="e.g., 500" /></div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={teamForm.selectedWorkerIds.length===0} className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 text-white text-sm font-medium disabled:opacity-50">Apply Punches</button>
              </div>
            </form>
          </motion.div>
        </div>
      ...
      {showAttendance && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowAttendance(null)}>
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0f0f1a] p-6" onClick={e => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-zinc-100 mb-1">Record Daily Punches</h2>
            <p className="text-sm text-zinc-500 mb-4">{showAttendance.name} — ₹{showAttendance.ratePerPunch}/punch</p>
            <form onSubmit={handleAttendance} className="space-y-3">
              <div><label className="block text-xs text-zinc-500 mb-1">Date *</label>
                <input type="date" value={attForm.date} onChange={e => setAttForm({ ...attForm, date: e.target.value })}
                  className="w-full px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-zinc-200 text-sm outline-none" /></div>
              <div><label className="block text-xs text-zinc-500 mb-1">Punches (Bricks Manufactured)</label>
                <input type="number" value={attForm.punchCount || ''} onChange={e => setAttForm({ ...attForm, punchCount: +e.target.value })}
                  className="w-full px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-zinc-200 text-sm outline-none" placeholder="e.g., 500" /></div>
              <div className="p-3 rounded-xl bg-orange-500/5 border border-orange-500/10">
                <p className="text-sm text-orange-400 font-semibold">Daily Earning: {formatCurrency(attForm.punchCount * showAttendance.ratePerPunch)}</p>
                <p className="text-[10px] text-zinc-600 mt-0.5">{attForm.punchCount} punches × ₹{showAttendance.ratePerPunch}/punch</p>
              </div>
              <div><label className="block text-xs text-zinc-500 mb-1">Notes</label>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 text-white text-sm font-medium">Save</button>
              </div>
            </form>
          </motion.div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowEditRate(null)}>
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="w-full max-w-sm rounded-2xl border border-white/10 bg-[#0f0f1a] p-6" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleUpdateRate} className="space-y-3">
              <div><label className="block text-xs text-zinc-500 mb-1">New Rate per Punch (₹)</label>
              ...
              <p className="text-[11px] text-zinc-600">Current rate: ₹{showEditRate.ratePerPunch}/punch</p>
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 text-white text-sm font-medium">Update Rate</button>
              </div>
            </form>
          </motion.div>
        </div>
      ...
      {showPayment && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowPayment(null)}>
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0f0f1a] p-6" onClick={e => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-zinc-100 mb-1">Pay Worker — {showPayment.name}</h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowHistory(null)}>
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="w-full max-w-2xl rounded-2xl border border-white/10 bg-[#0f0f1a] p-6 max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-zinc-100">{showHistory.name} — Daily Punch History</h2>
            ...
            <table className="w-full">
              <thead><tr className="border-b border-white/5">
                ...
                <th className="text-right text-[11px] text-zinc-500 font-medium px-3 py-2">Punches</th>
              ...
              <tbody>
                {workerDetail.attendance?.map((a: any) => (
                  <tr key={a.id} className="border-b border-white/5">
                    ...
                    <td className="px-3 py-2 text-right text-sm font-semibold text-orange-400">{a.punchCount}</td>
          ...
          <motion.div key={w.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            ...
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => { setShowEditRate(w); setRateForm(w.ratePerPunch); }}
                  className="flex items-center gap-1 text-[10px] text-zinc-600 hover:text-blue-400 transition-colors p-1">
                  <Pencil className="w-3 h-3" /> ₹{w.ratePerPunch}/punch
                </button>
            ...
            <div className="mt-3 grid grid-cols-3 gap-2">
              <button onClick={() => { setShowAttendance(w); setAttForm({ punchCount: 0, isPresent: true, notes: '', date: new Date().toISOString().split('T')[0] }); }}
                ...
                <Calendar className="w-3 h-3" /> Punches
              </button>
