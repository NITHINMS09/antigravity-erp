                  ...
                  <div className="space-y-3">
                    {form.items.map((item, i) => (
                      <div key={i} className="grid grid-cols-12 gap-3 items-end group">
                        ...
                        <div className="col-span-2">
                          <div className="px-4 py-3 rounded-xl bg-white/5 border border-transparent text-sm text-orange-400 font-black">
              ...
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                {p.paymentStatus !== 'paid' && (
                  <button onClick={() => { setShowPayment(p); setPayForm({ amount: p.dueAmount, method: 'cash', notes: '' }); }}
                    className="p-2 rounded-xl bg-white/5 hover:bg-emerald-500/10 text-zinc-500 hover:text-emerald-400 border border-transparent hover:border-emerald-500/20"><RupeeIcon className="w-4 h-4" /></button>
                )}
                <button onClick={() => handleDeletePurchase(p.id, p.purchaseNumber)}
                  className="p-2 rounded-xl bg-white/5 hover:bg-red-500/10 text-zinc-500 hover:text-red-400 border border-transparent hover:border-red-500/20"><TrashIcon className="w-4 h-4" /></button>
              </div>
            </div>
          </motion.div>
        ))}
